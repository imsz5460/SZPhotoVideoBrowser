...
@property (nonatomic, strong) CLGCDTimerManager *CLGCDTimerManager;
...
- (CLPlayerMaskView *) maskView{
    if (_maskView == nil){
        ...
        _CLGCDTimerManager = [[CLGCDTimerManager alloc] init];
        //计时器，循环执行
        [_CLGCDTimerManager scheduledDispatchTimerWithName:CLPlayer_sliderTimer
                                                             timeInterval:1.0f
                                                                delaySecs:0
        ...
        [_CLGCDTimerManager startTimer:CLPlayer_sliderTimer];
    }
    return _maskView;
}
/**状态栏*/
...
-(void)setToolBarDisappearTime:(NSInteger)toolBarDisappearTime{
    ...
    //定时器，工具条消失
    [_CLGCDTimerManager scheduledDispatchTimerWithName:CLPlayer_tapTimer
    ...
    [_CLGCDTimerManager startTimer:CLPlayer_tapTimer];
}
#pragma mark - 传入播放地址
...
- (void)pausePlay{
    self.maskView.playButton.selected = NO;
    [_player pause];
    [_CLGCDTimerManager suspendTimer:CLPlayer_sliderTimer];
}
#pragma mark - 播放
- (void)playVideo{
    ...
    if (_isEnd && self.maskView.slider.value == 1) {
        ...
        [_player play];
        [_CLGCDTimerManager resumeTimer:CLPlayer_sliderTimer];
    }
}
#pragma mark - 重新开始播放
- (void)resetPlay{
